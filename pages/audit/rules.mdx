...
These rules enforce the standards defined on the [Governance](/data-design/avo-tracking-plan/governance) page and are available on the Enterprise plan.

### All required custom fields are set

Checks if all events, event variants and properties have a value set for every [custom field](/data-design/avo-tracking-plan/governance/custom-fields) that is marked as required for that item type. Custom fields can be required per scope, so a field can for example be required on events while remaining optional on properties.

#### How to resolve

Fill in the missing custom field values in the Custom Fields section of the event, variant or property details, on a branch. Note that event variants don't inherit custom field values from their base event — required fields scoped to variants need to be filled in on each variant.

### All properties have a declared PII status

Checks for properties with an Undeclared [PII status](/data-design/avo-tracking-plan/governance/tagging-pii). Every property defaults to Undeclared until someone explicitly reviews it and marks it as either Not PII or PII, so this rule ensures every property in your tracking plan has been reviewed for personally identifiable information.
