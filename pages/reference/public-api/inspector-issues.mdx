...
Five behaviors are not visible anywhere in the response body, and each one produces a plausible-looking but wrong integration when it is assumed away. Four of them cut across both endpoints and are covered here:
...
One more is specific to the variations endpoint and is covered in its own section: [property names are the raw names the SDK sent](#property-names-are-raw-observed-names), not tracking-plan names.
