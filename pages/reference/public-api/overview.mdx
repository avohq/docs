...
- [Import Tracking Plan](/reference/public-api/import-tracking-plan): Import a tracking plan from a CSV file into a specific branch in your Avo workspace
- **Save Items** (alpha): Create tracking plan items programmatically via `/saveItems`. Requires a Bearer token with `write` scope. Currently under construction — [contact us](mailto:support@avo.app) for alpha access.
