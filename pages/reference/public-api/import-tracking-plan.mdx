...
The service account must have the `ImportCSV` action permission and branch-level access to the target branch.
...
| Value | Description |
| :-- | :-- |
| `addOnly` (default) | Only adds new events and properties. Nothing already in the tracking plan is modified. |
...
**Priority:** If both `schema` and `events` keys exist, Format A takes precedence.

#### Event-Level Fields (Format B only)
...
- Existing events are NOT modified (description, tags, sources, etc. remain unchanged)
