...
Available since version `3.1.0`. For production environments where bundle size matters (GTM, script tags, etc.), the lite entry point excludes property value validation, session filtering, and event deduplication. Use the full build in dev/staging environments for the complete [Inspector Debugger](/inspector/inspector-debugger) experience.
