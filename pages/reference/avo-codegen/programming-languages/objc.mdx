...
The actual parameters depend on your tracking plan setup, see the parameters explanation in [the reference below](/implementation/reference/objc#initAvo).
...
This method will call the `makeWithEnv:withApiKey:` callback in all the provided [destination interfaces](/implementation/destinations#destination-interface-callback-methods). It will also initialize the analytics SDKs of the legacy [Avo Managed destinations](/implementation/destinations#avo-managed-destination-legacy).
