...
This page documents the single-file Swift output. Avo can also emit Swift in a "library" shape that splits the runtime from per-event types — see [Library codegen](/implementation/library-codegen).
</Callout>
