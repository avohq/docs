...
This page documents the single-file Kotlin output. Avo can also emit Kotlin in a "library" shape that splits the runtime from per-event types — see [Library codegen](/implementation/library-codegen).
</Callout>
