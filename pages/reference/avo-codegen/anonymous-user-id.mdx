...
An optional `anonymousUserId` parameter is available on demand in all server languages, run `avo pull` with the `--forceFeatures ServerTrackingWithDeviceId` flag if you don't see it in your Codegen. You can contact [contact us](/help/troubleshooting) to have it enabled by default.
