...
Server side Codegen does not have a way to maintain user identity. That's why if you want to associate events with particular users before they identify, you need to pass the `anonymousUserId` string parameter to your Codegen. For Avo workspaces created after July 2023 an optional `anonymousUserId` parameter is available on demand in all server languages. For older workspaces, you can run `avo pull` with the `--forceFeatures ServerTrackingWithDeviceId` flag if you don't see it in your generated code and you can [contact us](/help/troubleshooting) to have it enabled by default.
