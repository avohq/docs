...
An optional `anonymousUserId` parameter is available on demand in all server languages, run `avo pull` with the `--forceFeatures ServerTrackingWithDeviceId` flag if you don't see it in your generated code. You can [contact us](/help/troubleshooting) to have it enabled by default.
