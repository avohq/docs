...
When an event has [triggers](/data-design/event-triggers), `get` with `type: "event"` returns each trigger with its full context, so an agent can see *when* the event fires — not just that a trigger exists. There are two trigger shapes:
...
Fields that aren't present are omitted — nothing is fabricated. An event with no triggers has no Triggers section. Trigger context is returned on the main branch and on branch lookups (`branchId` / `branchName`) alike, and event-variant lookups render their triggers the same way.
