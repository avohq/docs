...
- `itemType: "journey"` (or `source` / `destination` / `groupType`) combined with `query` or with content filters such as `tags` or `eventNames` — rejected; these types enumerate a whole branch or workspace and take only `branch` (and `pageToken`).
...
Claude first lists the branch's journeys with [`search`](#search) (`itemType: "journey"`) to find the journey's ID, then calls `get` with it. Journeys are addressed by ID only.
