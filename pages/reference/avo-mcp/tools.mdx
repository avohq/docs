...
<Callout type="info" emoji="🔒">
  Tools are grouped below by capability. Each tool lists the OAuth scope required. Write tools (`workflow`, `save_items`, `save_metrics`) require the `write` scope, which is requested as a separate consent step on first use.
</Callout>
...
- [`save_metrics`](#save_metrics) — create / update / archive metrics on a branch
...
Batch create, update, and archive metrics on a branch. Supports all six metric types: `Funnel`, `EventSegmentation`, `Proportion`, `Retention`, `CustomEvent`, and `Cohort`. A single call can mix metric types and operations, and can cross-reference newly-created events or metrics via temporary IDs.
...
|---|---|---|
| `branchId` | Yes | The branch to write to. Get it from `workflow`/`create_branch` or `list_branches`. |
| `metrics` | Yes | Array of metric items to apply. |
| `workspaceId` | No | Workspace ID |

The request is capped at **50 items** per call (stricter than `save_items`' 100-item cap because a single Cohort create can fan out to dozens of underlying actions).

### Item shape

Every metric item shares these fields:

| Field | Type | Notes |
|---|---|---|
| `op` | `"create"` \| `"update"` \| `"archive"` | Defaults to `"create"`. Metrics use **archive**, not remove. |
| `type` | `"metric"` | Optional, defaults to `"metric"`. Only metrics are handled here — `save_items` handles events and properties. |
| `name` | string | Required. Metric name. |
| `tempId` | string | **Create only.** Declares a temporary handle (e.g. `"m1"`). Reference it elsewhere in the same call as `"$tmp:m1"`. Returned back to you on `createdEntities` so you can map it to the allocated real ID. |
| `metricId` | string | Required on `update` / `archive`. |
| `description` | string | Create only. Metric description. |

**Create fields:**

| Field | Notes |
|---|---|
| `metricType` | Required. One of `Funnel`, `EventSegmentation`, `Proportion`, `Retention`, `CustomEvent`, `Cohort`. Immutable after create. |
| `items` | Required for non-Cohort metrics. Array of metric items whose shape depends on `metricType`. The server validates the shape. |
| `cohortConditions` | Required for `Cohort` metrics; rejected on non-Cohort types. Array of cohort conditions that reference events (by `eventId` or `$tmp:` ref) and property filters. Max 25 conditions per metric. |

**Update fields:** _(require `metricId`)_

| Field | Notes |
|---|---|
| `setName` | Rename the metric. |
| `setDescription` | Set the metric description. |
| `addItems` / `removeItems` | Non-Cohort: metric items to add, or item IDs to remove. |
| `addCohortConditions` / `updateCohortConditions` / `removeCohortConditions` | Cohort: conditions to add, conditions to update, or condition IDs to remove. |

`setType` is **rejected** — metric type is immutable post-create. To change a metric's type, archive the existing metric and create a new one.

**Archive:** `{ op: "archive", metricId: "..." }` archives the metric on the branch.

<Callout type="info" emoji="🔒">
  Cohort metrics require the `CohortMetricType` workspace flag to be enabled. If a Cohort create is rejected for that reason, ask a workspace admin to enable the flag in workspace settings.
</Callout>

<Callout type="info" emoji="💡">
  The per-type shape of `items` and the shape of `cohortConditions` are validated server-side. See the Avo schema documentation for the exact shapes; at a high level, non-Cohort `items` reference events and properties that participate in the metric, and each `cohortConditions` entry describes an event criterion plus optional property filters and time/frequency bounds.
</Callout>

### Temporary IDs (`tempId` / `$tmp:`)

Same semantics as `save_items`, with one addition: `$tmp:` references are resolved inside nested fields too — notably `items[].metricId`, `items[].eventId`, and `cohortConditions[].eventId`. This lets a Cohort metric reference an event created in the same batch, or a Funnel reference a property created in the same batch.

`createdEntities` entries include the `tempId` you originally declared, so a client that batched several creates can map each allocated real ID back to its handle.

### Example: create a Funnel metric
  ...
  "metrics": [
    {
      ...
      "name": "Signup Funnel",
      "metricType": "Funnel",
      "description": "Landing → Signup → Verified",
      "items": [
        { "eventId": "evt-landing" },
        { "eventId": "evt-signup" },
...
### Example: create a Cohort referencing an event created in the same call
  ...
  "metrics": [
    {
      ...
      "metricType": "Cohort",
      "cohortConditions": [
        { "eventId": "$tmp:activated_event", "withinDays": 7 }
      ]
    }
  ]
}
...
(The `$tmp:activated_event` reference here would be declared via a create-event item in a `save_items` call on the same branch, then looked up by its resolved ID. Metrics and events are written through separate tools; cross-batch `$tmp:` resolution lives within a single call.)

### Example: rename and redescribe a metric
  ...
  "metrics": [
    {
      "op": "update",
      "metricId": "met-abc123",
      "setName": "Checkout Funnel (v2)",
      "setDescription": "Updated to include Apple Pay step"
    }
  ]
}
```

### Example: archive a metric

```json
{
  "branchId": "br-abc123",
  "metrics": [
    { "op": "archive", "metricId": "met-abc123" }
  ]
}
```
...
- `createdEntities`, `updatedEntities`, `archivedEntities` — each entry has `name`, `entityId`, `entityType`, and (where the caller supplied one) `tempId`. Metrics use an `archivedEntities` envelope, not `removedEntities`.
- `errors` — per-item validation errors, each with the item index, name, type, and message.
- `warnings` — non-fatal notices, same shape as errors.
- `success` — overall boolean.
...
- Missing `write` scope — the client must re-authorize with `write`.
- `too many items (got N, max 50)` — batch is over the 50-item cap (checked pre-auth).
- `type_ cannot be changed during update. Archive and recreate instead.` — an update item carried `setType`.
- `cohortConditions` supplied on a non-Cohort metric, or omitted on a Cohort create.
- `cohortConditions exceeds max of 25 per metric spec`.
- Bounds violations on Cohort conditions (e.g. `frequency: 0` rejected with a clear message).
- Cohort create rejected because the `CohortMetricType` workspace flag is not enabled.
- Per-item validation failures — returned inside the `errors` array rather than failing the whole call.
