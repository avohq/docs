...
<Callout type="warning" emoji="🚧">
  **Write access is in closed beta.** The `read` tools are generally available; the `write` tools (`workflow`, `save_items`, and `save_metrics`) are enabled per-workspace. [Email us at support@avo.app](mailto:support@avo.app) if you'd like your workspace added to the beta.
</Callout>
...
- Create, update, and archive metrics (Funnel, EventSegmentation, Proportion, Retention, CustomEvent, Cohort) on a branch — including cohort conditions that reference events
...
| Create / update / archive metrics on a branch | [`save_metrics`](/reference/avo-mcp/tools#save_metrics) | `write` |
...
1. `workflow` with `action: "create_branch"` → `branchId` (or reuse an existing branch).
2. `save_metrics` on that `branchId` with one or more metric items. Pass `metricType` (`Funnel`, `EventSegmentation`, `Proportion`, `Retention`, `CustomEvent`, or `Cohort`) plus the per-type shape.
3. For cohort metrics, the `CohortMetricType` workspace flag must be enabled — ask a workspace admin if `save_metrics` rejects a Cohort create for that reason. Cohort conditions can reference events created in the same batch via `$tmp:` refs.
