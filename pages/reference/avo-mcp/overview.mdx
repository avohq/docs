...
For clients that support HTTP transport, point them at `https://mcp.avo.app/mcp`. Refer to your client's documentation for how to configure an HTTP-based MCP server.
...
All tools are workspace-scoped. The typical first-use sequence is:
...
4. `get_branch_code_snippets` with a `sourceId` — get exact before/after code diffs for each changed event
