...
The Avo MCP (Model Context Protocol) server exposes your Avo tracking plan to AI coding assistants. With it, tools like Claude Code, Cursor, and other MCP-compatible clients can read your tracking plan, explore branches, and generate correct analytics instrumentation — without you having to copy-paste event specs into the chat.
...
For clients that support HTTP transport and OAuth authentication, point them at `https://mcp.avo.app/mcp`. Refer to your client's documentation for how to configure an HTTP-based MCP server with OAuth.
...
Call `save_workspace` with your workspace ID. This persists the selection so every subsequent tool call uses it automatically — you won't need to pass `workspaceId` each time.
...
Once a workspace is saved, you can browse branches, look up sources, search for events, and get implementation guides directly in your AI coding session.
...
3. `get_branch_implementation_guide` with a `sourceId` — get a summary of what changed and what to implement
4. `get_branch_code_snippets` with a `sourceId` — get implementation snippets for each changed event (exact diffs for Codegen sources, pseudocode for manual sources)
