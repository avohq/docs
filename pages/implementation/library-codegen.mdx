...
> **Draft — internal review.** This page describes the library-shaped codegen output but does not yet include the customer-facing opt-in flag, the recommended Swift module / Kotlin Gradle layout, or the destination-wrapper details. Those open items are listed in [Questions / gaps](#questions--gaps) at the bottom. Don't link this page from customer-facing places until it's filled in.
...
1. **Drop the legacy `Avo` constructor.** Only the legacy `Avo(env:, customDestination:, ...)` constructor (Swift/Kotlin) and the legacy free-function `Avo.initAvo(config, destinations)` (TypeScript) are gone. In the new shape you keep using `Avo.initAvo(...)` for Swift and Kotlin — that's the generated extension on the library `Avo` class — and you switch to `Avo.init(config, codegenConfig)` for TypeScript with the codegen-bound config imported from `AvoConfig.ts`.
