...
Previously merged Avo branches can always be viewed back in time. That means you can always see what changes were made when, and by whom.

In this example we were looking at a team from iOS and Android. The exact same could apply for two or more teams working in parallel on tracking changes for a single platform, where each team would have their own Avo branch to work on, and would eventually merge their tracking implementations in git.
