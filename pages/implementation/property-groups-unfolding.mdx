...
The fact that we require to explicitly pass a `null` value for every `Sometimes Sent` property when it's not sent allows the compiler to catch a very common implementation related data bugs.
