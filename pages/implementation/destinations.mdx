...
You can use the destination interface with any analytics platform, please [contact usage](/help/troubleshooting) if you are unsure of what is
the best solution for you.

To get started using a destination interface you do the following:

1. Create a [Destinationusage](/data-design/define-sources-and-destinations#destinations-1Destination) in the Connections tab
2. Set the Destination Mode to Destination Interface in the [Codegen Setupusage](/data-design/define-sources-and-destinations#avo-codegen-setup) tab in your Source
3. Pull the updated code by doing `avo pull [--branch my-branch] [SourceName]` in the Avo CLI
4. Initialize Avo with a custom destination ([view example for JavaScriptusage](/implementation/reference/javascript#destination-interface-example))
5. Fill in the empty callback methods for sending the data to your analytics destination

## Destination Interface overview

We will be referencing [\_Codegen_usage](/implementation/avo-codegen-overview), [\_events_usage](/workspace/tracking-plan/events), [\_actions_usage](/workspace/tracking-plan/events#a-nameactionsa-actions) and other Avo concepts in this document. Please read [the Tracking Plan docusage](/workspace/tracking-plan) and subdocs to get familiar with Avo concepts.

With a destination interface you'll get access to callback methods for all the Avo actions. The Avo generated code triggers the callbacks when corresponding [Codegenusage](/implementation/avo-codegen-overview) are called.
...
> Learn how to attach actions to events in [the Tracking Plan sectionusage](/workspace/tracking-plan/events#actions).
...
- `logEvent`: All your analytics events are managed in the Tracking Plan in Avo. Each event gets a generated Avo function. Avo events can have a [Log Event actionusage](/workspace/tracking-plan/events#a-namelog-eventa-log-event) attached. This callback is invoked when an Avo function with Log Event action is called.
...
- `identify`: If you call an Avo Function for an event with the [Identify User actionusage](/workspace/tracking-plan/events#a-nameidentify-usera-identify-user) in the Tracking Plan, this callback will be invoked. When calling an Avo Function that includes the Identify User action you'll need to provide a user ID.
...
- `unidentify`: If you call an Avo Function for an event with the [Unidentify User actionusage](/workspace/tracking-plan/events#a-nameunidentify-usera-unidentify-user) in the Tracking Plan, this callback will be invoked.
...
- `revenue`: If you call an Avo Function for an event with the [Log Revenue actionusage](/workspace/tracking-plan/events#a-namelog-revenuea-log-revenue) in the Tracking Plan, this callback will be invoked.
...
- `page`: If you call an Avo Function for an event with the [Log Page Viewusage](/workspace/tracking-plan/events#a-namelog-page-viewa-log-page-view) action in the Tracking Plan, this callback will be invoked.
...
- `setUserProperties`: If you call an Avo Function for an event with the [Update User Properties actionusage](/workspace/tracking-plan/events#a-nameupdate-user-propsa-update-user-properties) in the Tracking Plan, this callback will be invoked.
...
- `setGroupProperties`: You can update various group properties along with events in the Tracking Plan. If you call an Avo Function for an event with the [Update Groups actionusage](/workspace/tracking-plan/events#update-groups) in the Tracking Plan, this callback is invoked.
