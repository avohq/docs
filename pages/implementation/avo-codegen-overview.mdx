...
For Kotlin, Swift, and TypeScript sources, Avo can emit code in a "library" shape that splits the runtime from the per-event types. See [Library codegen](/implementation/library-codegen) for the file layout, quick starts, and migration steps from the single-file output.
