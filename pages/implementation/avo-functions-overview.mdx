...
We generate the files and you can download them either with <Link passHref href="/implementation/cli"><a>the Avo CLI</a></Link> or in the Avo web interface in <Link passHref href="/workspace/implement"><a>Codegen tab</a></Link>.
...
- `inspector` (not shown in the example) - instance of Avo Inspector to automatically send Avo Codegen data to Avo Inspector (on the supported platforms)
...
For example if you have an event defined with name `App Opened` connected to your source, Avo Codegen interface will look like
