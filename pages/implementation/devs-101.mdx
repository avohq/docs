...
Avo Functions are type safe analytics wrappers that can work with any analytics tool you already 
use or plan on using, whether it's any of the third-party event logging SDKs (e.g. 
...
that you have full control over how you implement. You provide a simple Destination Interface 
object when initializing Avo Functions, so that any time an Avo Function calls an analytics 
logging method (e.g. logEvent, setUserProperties, etc), you can govern exactly what analytics 
SDK method should be called.

In the <Link passHref href="/workspace"><a>Avo Tracking Plan</a></Link> you can configure which analytics actions 
(for example you might want to update user properties or log revenue along with logging an event)
should be triggered with an Avo Function, and the Avo Functions 
...
This is compatible to Segment's "device mode". With Segment, you can choose 
between <Link passHref href="https://segment.com/docs/guides/intro-impl/#connection-modes"><a>_Cloud-mode_ or _Device-mode_</a></Link>:
- In _Cloud-mode_, your events go through Segment servers. Your devices send the events to the 
Segment cloud (aka the Segment Servers) and the Segment cloud sends them to the analytics destinations.
- In _Device-mode_ the events go directly from your devices (whether it's a front end client 
or a server) to the analytics destination.

In this regard, Avo works like Segment's _Device-mode_.

## Avo Inspector: Tracking Observability

### What is Avo Inspector?

Avo Inspector is a tracking observability solution that's easy to install. Avo Inspector analyzes tracking calls made by your app 
and shows an audit in the web dashboard. It gives you an instant review of your current state of tracking, 
and an ongoing observability going forward.
