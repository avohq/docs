...
Avo Inspector includes an SDK that extracts tracking plan information from your tracking calls. It provides methods to get the shape of an event you are going to send to the analytics platform and then to send that shape alongside the event name for analysis.
