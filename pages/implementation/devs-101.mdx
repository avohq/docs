...
object when initializing Avo Functions SDK, so that any time an Avo Function calls an analytics 
logging method (e.g. logEvent, setUserProperties, etc), you can govern exactly what analytics 
SDK method should be called.

In the <Link passHref href="/workspace"><a>Avo Tracking Plan</a></Link> you can configure which analytics <Link passHref href="/workspace/tracking-plan/events#a-nameactionsa-actions"><a>actions</a></Link> should be triggered with an Avo Function (for example you might want to update user properties or log revenue along with logging an event), and the Avo Functions 
...
> This is compatible to Segment's "device mode". With Segment, you can choose between <Link passHref href="https://segment.com/docs/guides/intro-impl/#connection-modes"><a>_Cloud-mode_ or _Device-mode_</a></Link>:
> - In _Cloud-mode_, your events go through Segment servers. Your devices send the events to the Segment cloud (aka the Segment Servers) and the Segment cloud sends them to the analytics destinations.
...
Avo Inspector is a tracking observability solution that's easy to install. Avo Inspector analyzes tracking calls made by your app and shows an audit in the web dashboard. It gives you an instant audit of your current state of tracking, 
