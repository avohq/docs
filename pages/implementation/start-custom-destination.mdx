...
- When you want to have controll over the initialization and the lifecycle of the Analytics SDK
...
- `identify`: You can add the Identify User action to an event in the Trackin Plan in Avo. When calling an Avo function that includes the Identify User action you'll need to provide a user ID. Calling an Avo function that has the Identify User action will trigger this callback.
...
```
where `custom_destination` parameter is the name of the custom destination in Avo.
