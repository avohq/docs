...
var customDestination = {
  ...
  addToGroup: function (groupType, groupId, groupProperties) {
    // Segment example
...
let customDestination: Avo.avoCustomDestination = {
  make: (_env: Avo.avoEnvType) => (),
  logEvent: (eventName: string, eventProperties: Js.Json.t) => ();
  setUserProperties: (_userId: string, _userProperties: Js.Json.t) => ();
  identify: (_userId: string) => ();
  unidentify: () => ();
  addToGroup: (groupType: string,
    groupId: string,
