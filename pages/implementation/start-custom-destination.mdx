...
- Update Group info
  ...
  With this action youi can update the Goup properties of a particular group and assign currently identified user to a group.
...
- `setGroupProperties`: You can add various group properties to events in the Avo dashboard. When an Avo function with attached groups and group properties is called, this callback is triggered.
...
- `logEventWithGroup`: this call is same as the `logEvent` call described above, except it also associates this particular event with a group.
...
var customDestination = {
  ...
  logEventWithGroup: function (eventName, eventProperties, groupType, groupId) {
    // Mixpanel example
    mixpanel.track_with_groups(eventName, eventProperties, {
      groupType: groupId,
    });
    // Amplitude example
    amplitude
      .getInstance()
      .logEventWithGroups(eventName, eventProperties, { groupType: groupId });
  },
};
```
