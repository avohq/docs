...
If you already have an analytics SDK running in our app, it's initialized and already receiving data, you should go with Option 1.
If you don't have any existing tracking you can pick any option.
...
If you use multiple analytics tools, you should repeate this process and create a destination for every analytics tool.
...
If we are using an Avo-managed tracking destination all we need to do is to import the corresponding SDK.

If we are using Custom Destinations we define an interface for each destination, where we connect it to our analytics SDK:
...
Detailed instructions for all other programming languages and platforms supported by Avo <Link passHref href="/implementation/start-custom-destination"><a>can be found here</a></Link>.

### Send an event

Now we're ready to start implemening reliably analytics tracking with Avo Functions. Every Event defined in our <Link passHref href="/workspace/tracking-plan"><a>Trackin Plan on avo.app</a></Link> has it's own functin in the Avo Functions file.
