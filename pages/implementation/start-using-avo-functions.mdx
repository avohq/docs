...
If a source doesn't already exist for your app or website, click "Add Source" to create a new Source. Give it a name and define what development platform and programming language you'll be using. This configuration is used to tailor the Avo Codegen to your stack.
...
_Step 1: **Implement a bridge interface** between your existing analytics SDK instance and the Avo Codegen._

- This enables you to use the same analytics SDK instance across all tracking calls (using the same session IDs and any other details that your analytics instance keeps track of), so that your users will be identified in the same way across both the tracking implemented with Avo Codegen and your existing analytics implementation.
...
> Avo code-generates a file based on the Tracking Plan in your Avo workspace. The file contains a function per each Event in the Tracking Plan that you use to send your events. We call these functions Avo Codegen. Each Avo Function validates the event data before passing it forward to your Destinations.
>
> You can think about the Avo Codegen like a utility file for your analytics. It should be placed somewhere in the source code directory of your Source and checked into git. Every time the tracking plan in Avo is updated, the Avo Codegen is updated.
>
> To make the workflow of updating the Avo Codegen as easy as possible we built the Avo CLI. By running `avo pull [--branch my-branch] [SourceName]` the CLI will update the file containing the Avo Codegen automatically.
...
Now you can pull the Avo Codegen from your Avo workspace. When you run `avo pull` for the first time it will also initialize Avo in your project by adding an `avo.json` file, containing a pointer to your Avo workspace, source and current branch. This file should be checked into git along with the Avo Codegen file.
...
Before we can start using the Avo Codegen to log events we first need to initialize Avo by calling the `initAvo` function exported from the file. Avo is usually initialized in the same place you initialize all your other analytics SDKs, or as soon as possible in the runtime of your app.

First we import the Avo Codegen and set up the destinations.
...
Now we're ready to start implementing reliably analytics tracking with Avo Codegen. Every Event defined in our <Link passHref href="/workspace/tracking-plan"><a>Tracking Plan on avo.app</a></Link> has it's own function in the Avo Codegen file.

Let's say we have defined the "App Opened" event in our Tracking Plan in Avo, with two Event Properties, "Client" and "Screen Name". Here's how we would send the event using Avo Codegen:

```typescript
Avo.appOpened({ client: 'Web', screenName: 'Home Screen' });
```

> On some server platforms Avo Codegen functions are async, e.g. on Node.js they return a Promise and on C# you can await on the functions
...
You can find tailor made documentation for your Avo Codegen, based on your Tracking Plan, in the <Link passHref href="/workspace/implement#a-nameimplement-actionsa-implement-events"><a>Codegen tab</a></Link> in [your Avo workspace](https://www.avo.app/welcome).
