...
If a source doesn't already exist for your app or website, click "Add Source" to create a new Source. Give it a name and define what development platform and programming language you'll be using. This configuration is used to tailor Avo Codegen to your stack.
...
_Step 1: **Implement a bridge interface** between your existing analytics SDK instance and Avo Codegen._
...
>
> You can think about Avo Codegen like a utility file for your analytics. It should be placed somewhere in the source code directory of your Source and checked into git. Every time the tracking plan in Avo is updated, Avo Codegen is updated.
>
> To make the workflow of updating Avo Codegen as easy as possible we built the Avo CLI. By running `avo pull [--branch my-branch] [SourceName]` the CLI will update the file containing Avo Codegen automatically.
...
Now you can pull Avo Codegen from your Avo workspace. When you run `avo pull` for the first time it will also initialize Avo in your project by adding an `avo.json` file, containing a pointer to your Avo workspace, source and current branch. This file should be checked into git along with Avo Codegen file.
...
Before we can start using Avo Codegen to log events we first need to initialize Avo by calling the `initAvo` function exported from the file. Avo is usually initialized in the same place you initialize all your other analytics SDKs, or as soon as possible in the runtime of your app.

First we import Avo Codegen and set up the destinations.
...
Now we're ready to start implementing reliably analytics tracking with Avo Codegen. Every Event defined in our <Link passHref href="/workspace/tracking-plan"><a>Tracking Plan on avo.app</a></Link> has it's own function in Avo Codegen file.
