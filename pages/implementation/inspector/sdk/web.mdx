...
The Web SDK maintains some shared state across the tracking calls and stores the data for batching, which makes it not suitale for web workers usage.

If you have event tracking in Web Workers, we recommend to set up a listener in the main thread and send your event data there, where an inspector instance will be listening for it.
