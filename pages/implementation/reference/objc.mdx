...
+ (void)initAvoWithEnv:(AVOEnv)env
  ...
  systemProprty0:(nullable NSNumber *)requiredStringSystemProperty
  systemProperty1:(BOOL)optionalStringSystemProperty
  customDestDestination:(nonnull id<AVOCustomDestination>)customDestDestination
  strict:(BOOL)strict
  noop:(BOOL)noop;
```
...
`(nonnull id<AVOCustomDestination>) customDestDestination`: object, required if target source has a custom destination set up in Avo. Otherwise not present.
