...
Creates the Avo object that will be used to track. This method will call the `make(env, apiKey)` callback in all the provided destination interfaces. It will also initialize the analytics SDKs of the legacy managed destinations.
...
`destination: AvoCustomDestination`: object, each destination you are sending events to gets a separate parameter in the init function with hooks that the Avo generated code will trigger, unless you are using the legacy Avo managed destinations.

```swift
public protocol AvoCustomDestination {
    func make(env: AvoEnv, apiKey: String) // This method is optional, you can skip it if yo've already initialized your Analytics SDK
