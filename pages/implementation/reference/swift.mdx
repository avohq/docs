...
Because of the nature of the code generation Avo files can sometimes contain warnings, that do not affect the execution of the code, but may distract you from the product development.
To avoid it we suggest to disable the warning in the Avo generated files.

To do that:

- Select the file you want to suppress warnings for in the Xcode project navigator.
- Click on the file to open its details view in the right-hand pane.
- In the details view, click on the "Build Phases" tab.
- In the "Compile Sources" section, find the file you want to suppress warnings for and click on it to select it.
- Click on the "Compiler Flags" column next to the selected file, and add the `-w -Xanalyzer -analyzer-disable-all-checks` flags to suppress warnings.
- Save your changes, restart Xcode and rebuild your project.

<img
  src={require('../../../images/suppress-warnings-swift.png')}
