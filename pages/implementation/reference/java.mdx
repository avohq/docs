...
public class Destination implements ICustomDestination {
    ...
    public void revenue(String userId, double amount, Map<String, Object> eventProperties) {
        // Note: Segment does not provide a native method for revenue tracking, so we send an event instead. Other SDKs may have a dedicated revenue tracking method.        
