...
`destination`: object, each destination you are sending events to gets a separate parameter in the init function with hooks that the Avo generated code will trigger, unless you are using the legacy Avo managed destinations.

```javascript
{
  [make]: function (env, apiKey), // This method is optional, you can skip it if yo've already initialized your Analytics SDK
  ...
  // The following methods are used for group analytics, learn more about group analytics <Link passHref href="/data-design/groups"><a>here</a></Link>
