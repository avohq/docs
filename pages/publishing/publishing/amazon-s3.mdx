...
| **Folder Prefix** (optional) | A path segment placed in front of every object key, e.g. `avo-exports`. |
...
- **403 Access Denied**: The IAM user doesn't have `s3:PutObject` on the target bucket or folder prefix. Confirm the permission policy is attached to the user and that its `Resource` ARN matches your bucket, e.g. `arn:aws:s3:::<your-bucket>/*`.
