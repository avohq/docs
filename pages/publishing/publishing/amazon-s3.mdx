...
- Conditioning on `sub` rather than `aud` is deliberate: `aud` is whatever the caller requests when minting the token, so it isn't an identity proof, whereas AWS validates `sub` independently and the caller can't set it.
