...
*Publishing your tracking plan to Adobe Experience Platform*
...
## Configuring Adobe Experience Platform integration

To enable the direct integration with Adobe Experience Platform, navigate to the Publishing Screen in your Avo workspace, click “Add Integration” and select “Adobe Experience Platform. See details in our [setting up and configuring a publishing integration docs](publishing/publishing/overview#setting-up-and-configuring-a-publishing-integration)
...
To publish your Avo tracking plan to Adobe Experience Platform, navigate to the details of Adobe Experience Platform integration that you just created and click the "Publish" button. Enable Auto Publishing to trigger an automatic publish whenever a branch has been merged.

![Avo Publishing Integration](/images/adobe-experience-platform-integration.png)

You can also download an XDM file, which you can import into Adobe Experience Platform.

### What Is Included In The Publish to Adobe Experience Platform?

On publish, Avo will fetch all your existing XDM schemas and field groups from Adobe Experience Platform, compare them with your Tracking Plan and publish new and updated XDM schemas. By default Avo will publish all events on the branch that you are on, but you can configure the publish to be [filtered by Sources, Destinations and Tags](/publishing/publishing/overview#filtering-events-for-publishing).
...
The `Log Event` action on Avo events map to Adobe Experience Event schemas. Below are the components of Adobe Experience Platform schema and how Avo populates it for a `Log Event` action:
...
The `Update User Properties` action on Avo events map to Adobe Profile schemas. Below are the components of Adobe Experience Platform schema and how Avo populates it for a `Update User Properties` action:
...
**Breaking changes** are handled by creating a new schema or a field group. You will need to update your Adobe Experience Platform setup to reference the new schema or field group.

Learn more about the [breaking changes in Adobe Experience Platform](https://experienceleague.adobe.com/en/docs/experience-platform/xdm/schema/composition#evolution)
