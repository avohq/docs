...
For Webhook integrations, you can enable "Only send changed events" in the Events Filter section. When enabled, only events that are new or changed on the merged branch are included in the payload, along with a `removedEvents` field listing events that were archived or lost their filtered sources. This is useful for downstream consumers that only need to process incremental changes. See [Webhook Publishing](/publishing/webhook-publishing#only-send-changed-events) for more details.
