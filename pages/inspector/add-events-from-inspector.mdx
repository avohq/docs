...
The Inspector compares ingested events and properties against the tracking plan and detects if there are any events or properties missing from the tracking plan. You can add missing events or properties from both the [Issues](/inspector/inspector-issues-view) and [Events](/inspector/inspector-events-view) views. 

## Import events and properties in bulk from Inspector Issues view

In addition to providing the "Add to Tracking Plan" button for events not present in the tracking plan within the [Issue Status view](/inspector/inspector-issues-view#event-info),  you can review all of the events and properties missing from the tracking plan.

If there are events unexpected by the tracking plan detected in Inspector Issues, there will be a "Add X new events" button at the upper right of the main Inspector Issues table.
...
Once you click the button, a "Review import data" screen will pop up with the following information on top:

- The number of unexpected events and associated sources found by Inspector
- The number of Tracking Plan Audit alerts triggered by the unexpected events and properties
