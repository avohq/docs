...
<Callout type="info">
  The GTM Server template supports event spec validation and stream ID in dev and staging environments. Regex validation is not supported in the GTM Server sandbox and is silently skipped. Encryption is not supported due to sandbox limitations — property values are not sent to Avo from the server-side template.
</Callout>
