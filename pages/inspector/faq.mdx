...
No, you don’t! The <Link href="/inspector/sdk"><a>Avo Inspector SDK</a></Link> is designed to not require any changes to your existing tracking code. <Link href="/inspector/sdk"><a>Read more on setting up the SDK here</a></Link>.
...
The <Link href="/inspector/sdk"><a>Avo Inspector SDK</a></Link> analyzes your user data and delivers metadata about the user data to Avo’s servers. Based on that metadata we build your tracking plan and <Link href="/inspector/issue-identifier"><a>alert on any issues</a></Link>.

### Does Avo batch events in the SDK?

The Avo Inspector SDKs does batch events before sending metadata about them to Avo’s servers. The details of the batching may vary between platforms, <Link href="/inspector/sdk"><a>read more in the docs for the SDKs</a></Link>.
