...
Events can be filtered using the `eventPrefixesToExclude` and `eventsToInclude` fields.

Both fields should be JSON arrays of strings.

Any event whose name starts with one of the prefixes specified in `eventPrefixesToExclude` will be ignored and not reported to the Inspector — unless the full event name is explicitly included in the `eventsToInclude` array.

We provide default values that you can customize as needed.

The same logic applies to event properties, using the `propertyPrefixesToExclude` and `propertiesToInclude` fields.
