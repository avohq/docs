...
<Link href="/quickstart#validate-current-events">
  <a>Add the `validateAvoEvent` function</a>
</Link> at the location in your code base that handles the emission of analytics
events. Merge that code change into your git master to ensure that every time a user
interacts with your product in development, event structures are being logged into
Avo.
