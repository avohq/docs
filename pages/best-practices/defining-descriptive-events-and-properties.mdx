...
- <a href="#descriptions">Descriptions</a> that provide important information beyond the event and property names
- <a href="#property-constraints">Property Constraints</a> that limit the property values to the ones that are allowed
...
For <Link to="/workspace/tracking-plan#event-defintion">events</Link> we recommend adding a description to an event with all the information required for a developers to trigger an event by the same user action for all code paths, platforms and products.
...
<Link to="/workspace/tracking-plan#property-types-and-constraints">Property constraints</Link> are very useful to ensure that the correct spelling or allowed numerical values are sent with the event.
