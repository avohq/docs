...
> [Use Inspector to improve your tracking plan health](https://www.avo.app/docs/data-design/start-using-inspector)
...
<center>
  <img
        width="100%"
        src={require('../../images/workflow/5.validate/validate-dashboard.png')}
...
3. In the Inspector dashboard you can see all the events that were  sent from the selected environment during the select time period, and whether they fit the tracking plan or not. If any discrepancies were found, you can click into the event to learn more about the issue.
...
>[Inspector implementation status](https://www.avo.app/docs/workspace/tracking-plan/implementation-status#inspector-implementation-status)

# Validate implementation with Avo Functions

For all tracking implemented with Avo Functions, you can validate if the implementation is according to the tracking plan using multiple methods.

If you’re not using Avo Functions already, you can learn more about how to get started here:

>[Quickstart: Avo Functions](https://www.avo.app/docs/implementation/start-using-avo-functions)
...
>[What's happening inside the Avo Functions](https://www.avo.app/docs/implementation/avo-functions-overview#whats-happening-inside-the-avo-functions)

**Validate with the Avo debugger**

The Avo Debugger is an addition on top of Avo Functions that gives you better visibility into which events are being sent, which properties are attached, and whether any issues were detected or not.
...
>[Visual debuggers](https://www.avo.app/docs/explore-tracking-plan/start-using-visual-debuggers)
...
>[Avo in CI/CD](https://www.avo.app/docs/implementation/avo-in-the-ci)
...
>[Avo Functions implementation status](https://www.avo.app/docs/workspace/tracking-plan/implementation-status#avo-functions-implementation-status)


## What’s next?

>#### <Link passHref href="/workflow/merge-publish"><a>6. Merge branch and Publish</a></Link>
