...
> The inspector on production is designed to handle large amounts of data, therefor it has up to 2 hour delay from receiving events to displaying them on a dashboard. This means the "last 1 hour" dashboard on production could be showing events that came in up to 2 hours ago. Events on dev and staging have no delay in processing will be immedietly shown on the dashboard.
