...
> The inspector on production is designed to handle large amounts of data and has up to 2 hour delay from receiving events to displaying them on a dashboard.

> Events on dev and staging have no delay in processing will be immedietly shown on the dashboard.
