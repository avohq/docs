...
Items can be added from one branch to another by pulling or pushing. In both cases, the item retains the same ID across branches, preventing direct merge conflicts.
- [**Pushing an item**](#pushing-items-to-another-branch): Adds an item from the current branch to another branch.
...
Events, properties, and property bundles can be pushed from the current branch to another branch using the[Tracking Plan Changes view](/data-design/branches#review-branch-changes). 

In this view, each changed item appears individually. For items eligible for pushing (events, properties, and property groups), a button in the top right corner initiates the push process.
