...
Branch status makes it transparent at which step the peer-review process is and when it is changed, notifications are sent out to all branch collaborators.
...
- **Changes requested**: Indicates that a reviewer has requested changes to the branch before it can be approved. 
