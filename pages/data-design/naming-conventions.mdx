...
**_What is Avo Codegen?_** Avo Codegen produces type safe code for implementing analytics. The "data designer" specifies the event structure in the Avo app, and which platforms should send the event, and then the developer who implements analytics can use Avo Codegen per each analytics event.
...
_Also covered in <Link passHref href="/data-design/global-namespace"><a>Global namespace for events and properties</a></Link> _
...
2. **[Inspector](https://www.avo.app/inspector) indicates "rogue" events and properties that don't match your tracking plan**
   ...
   > \*_ It's impossible to ship events or properties with names that don't match your tracking plan when you implement using Avo Codegen, but, when companies adopt Avo, they often have multiple product teams and years of analytics tracking code in place._

   1. For adoption, we recommend adopting Avo team by team (not flag-day switch-over for entire company). This means: While some teams will implement with type-safe Avo Codegen, other teams and codebases will still implement "the old way". Which means, implementation errors will slip through 😅
   2. For replacing old tracking code, we recommend the "scout rule"; always leaving the analytics code better than when you found it, and porting analytics code to Avo Codegen as it makes sense.
