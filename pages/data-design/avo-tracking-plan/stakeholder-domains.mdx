...
In the Events table, you'll see **Stakeholders** and **Owner** columns that show which teams are stakeholders in each event and which team owns it.

### Filtering by stakeholders and owners

You can filter the events table by both stakeholder teams and owners to create focused views:

- **Stakeholders filter**: Shows events where the selected teams are stakeholders (depend on the events)
- **Owners filter**: Shows events owned by the selected teams (responsible for maintaining the events)

For complete details on all available filtering options and how to use them effectively, see [Filtering and organizing events](/data-design/avo-tracking-plan/events#filtering-and-organizing-events).
