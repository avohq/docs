...
Add the Log Page View action to events that are sent when the user lands on a page (web) or a screen (mobile). In addition to tracking the event, the destination will also register a page or screen view, so page analytics in the destination (page view counts, page dimensions, navigation reports) are populated from the same Avo Function call.

Adding the action does three things:

1. The generated Avo Function gets a required **page name** parameter.
2. Avo Codegen calls the page view method of each destination, right before the regular track call.
...
When an event has the Log Page View action, Avo Codegen calls the `logPage` callback of every [destination interface](/reference/avo-codegen/destinations#destination-interface-callback-methods) the event is sent to, passing the page name and the event properties, and then calls `logEvent` if the event also has the Log Event action. If you only want the page view and not the event, remove the Log Event action from the event.
