...
Some analytics destinations have revenue tracking separate from event tracking. Add the *Log Revenue* action to indicate that the event represents a successful purchace and should be used to track revenue. When using Avo Codegen this will result in your event funtion receiving an additional _amount_ parameter that will then be passed on to the `revenue` function in your destination interface. Read more on it in [the Codegen docs](/reference/avo-codegen/destinations#destination-interface-callback-methods).

##### Log Revenue in legacy Codegen destinations

Below are details for what Avo Codegen automatically does for each analytics platform. Note that this only applies to legacy Avo destinations and does not apply to destinations using the destination interface.
