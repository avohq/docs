...
New events can be created from the Events section by clicking the "New Event" button at the top of the view. They can also be created in context from a [Metric](/data-design/avo-tracking-plan/metrics) or a [Property](/data-design/avo-tracking-plan/metrics).
...
<Callout type="info" emoji="💡"When creating a new event you will receive guidance on how to name the event in a way that adheres to the [naming convention: How to Pick Your Events and Properties](/audit/advanced-event-naming-rules) defined for your workspace.</Callout>
...
1. **Inline Table Editing**: Properties, categories and tags can be edited directly in the table by clicking on individual cells.

3. **Batch Editing**: Select multiple events to apply bulk updates to shared attributes like sources, properties, and stakeholders.
