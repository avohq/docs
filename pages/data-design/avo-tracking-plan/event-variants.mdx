...
- remove properties: click the triple-dot icon next to the property and select Remove property on this variant.
- add properties: click the `+ Add Property to Variant` button and select the property you want to add.
- pin property or override existing pin: in the triple-dot menu, choose `Pin Property Value on Variant`
- override property presence: click the presence label in the variant details view
...
We’re stoked to introduce event variants to Avo and can’t wait to see how you use them to better define your tracking plan. As with any new features, there might be some things we need to iron out and iterate on. We are currently working on adding more variations like sources and assigning variant specific rules. If you come across any problems or find something missing for your use case, we’d love to hear about it. Reach out right here in the chat or send us a line at [hi@avo.app](mailto:hi@avo.app).
