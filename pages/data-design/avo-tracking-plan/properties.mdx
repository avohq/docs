...
- Add allowed values, change a property's description or type, or archive a property — all via [`save_items`](/reference/avo-mcp/tools#save_items) `op: "update"` / `op: "archive"`
...
- The [categories](/data-design/guides/organizing-metrics-and-events#categories) the property belongs to (optional)
- The [tags](/data-design/guides/organizing-metrics-and-events#tags) the property is associated with (optional)
- The [PII status](/data-design/avo-tracking-plan/governance/tagging-pii) of the property – whether it carries personally identifiable information (Enterprise plan)
- The [custom field](/data-design/avo-tracking-plan/governance/custom-fields) values defined for your workspace (Enterprise plan)
...
Whether a system property is sent from a given source can be configured on the source itself as Always, Sometimes or Never. This affects how Inspector validates the property and whether Codegen includes it in the initialization method. See [System Properties in the source overview](/data-design/avo-tracking-plan/define-sources-and-destinations#system-properties) for details.
...
For a broader overview of how bundles fit into organizing your tracking plan, see [property bundles](/data-design/guides/organizing-metrics-and-events#property-bundles).
...
You can configure when Properties are required on a per-event/variant and per-source basis. That means Avo will not give you an error if that property is missing from an event, variant or source which the property is attached to, in other words it will sometimes be sent.
