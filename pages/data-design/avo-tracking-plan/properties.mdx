...
#### Event and source specific allowed values

<Callout type="info">
  Event and source specific property rules are available on the Team plan.
</Callout>

When a property is sent from multiple events and sources, you can configure the allowed values granularly for each event and each source.

It can be configured both from the property details

// TODO new screenshot

![Image shows how allowed property values can be enabled or disabled on each event and source from property details](/images/property_enum_source_specific.png)

And from the event details

// TODO new screenshot

![Image shows how allowed property values can be enabled or disabled on specific event and the event sources from the event details](/images/property_enum_source_specific.png)
