...
A [journey trigger](/data-design/guides/event-triggers) are the same as [event triggers](/data-design/guides/event-triggers) except they also connected to journeys, not only events and variants. Journey triggers can also have more than one image attached to a single trigger while event triggers are only attached to one image.
...
The journey trigger is automatically added to the event or variant details, with the image from the journey step. An journey trigger can also be connected to a source to specify which sources the trigger applies to. Open the journey trigger details from the event or variant details to do that. See more in [Journey triggers and use cases](/data-design/guides/event-triggers).

#### Adding Journey triggers

To add an journey trigger to a journey, you can:
...
To add a property condition to an journey trigger:
...
Connections link a [journey step](/data-design/avo-tracking-plan/journeys#journey-step) to an [journey trigger](/data-design/avo-tracking-plan/journeys#journey-trigger). Add connections by clicking the node on either side of the journey step and dragging it to a node on either side of the trigger.
