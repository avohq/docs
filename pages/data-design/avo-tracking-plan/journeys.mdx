...
3. In Figma, copy the link to what you want to import — a whole file, a section, or a single frame:
...
4. Paste the link into the dialog in Avo.

   [Screenshot: Paste Figma URL dialog]

5. Click **Fetch frames**. Avo retrieves the file metadata from Figma and displays a visual frame picker showing all top-level frames available in the file.

   [Screenshot: Frame picker modal showing Figma frames as thumbnails]

6. Select one or more frames to import. You can select individual frames or use select-all.

7. Click **Import**. Avo downloads each selected frame as an image, uploads it, and creates a journey step for each one.
...
Make sure you copied the link from a Figma design file using one of the methods above — share link, `Cmd/Ctrl+L`, or right-click. Links from Figma's prototype view, inspect panel, or other tools (such as FigJam boards) are not supported.
