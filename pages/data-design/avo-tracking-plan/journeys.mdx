...
Journey triggers are the same as [event triggers](/data-design/guides/event-triggers) except they also connected to journeys, not only events and variants. Journey triggers can also have more than one image attached to a single trigger while event triggers are only attached to one image. See more in [Triggers and use cases](/data-design/guides/event-triggers).
