...
Once everyone is aligned, you can copy <Link passHref href="/workspace/implementation-instructions"><a>implementation instructions</a></Link> from the review screen and share them with a developer for example via Jira.
...
At this point we recommend to have automatic tracking plan publishing enabled. <Link passHref href="/data-design/start-collaborating"><a>Learn how to do it here</a></Link>. 
...
### Using Avo functions
For teams that are using Avo functions to implement their events, the typical implementation workflow looks like this:
...
3. Use the Avo functions provided in the generated code to implement your events in your source code. The <Link passHref href="/workspace/implement#a-nameimplement-actionsa-implement-events"><a>Functions tab</a></Link> in your workspace provides code snippets for each event.

<center>
  <img
    src={require('../../images/best-practices/day-to-day-workflow/implementation-instructions.png')}
    width="100%"
    alt="Functions reference"
  />
</center>
...
1. Open a git branch – if you have multiple development platforms (sources in Avo), we recommend having one git branch per source for each Avo branch.
2. Use the <Link passHref href="/workspace/implementation-instructions"><a>implementation instructions</a></Link> provided in the review screen as a guide for what needs to be changed or added
...
- <Link passHref href="/implementation/avo-functions-overview#data-validations">
