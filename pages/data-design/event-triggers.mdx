...
> 💡 Note that Event Triggers are only available on the Startup, Growth and Enterprise plans.
...
4. In the trigger modal that appears you are prompted to:
   ...
   - Select the sources that the trigger applies to
     - You can select "source independent" for the trigger to be applied to the event, independent of sources. Then it will be included in the implementation instructions and Avo functions for all sources on the event.
...
<center>
  <img
    src={require('../../images/best-practices/triggers/single-trigger.png')}
    width="75%"
