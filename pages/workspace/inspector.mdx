...
The first step to start using the Inspector, is installing it on your sources. Inspector shares sources with the workspace sources and they are all set up in the Connection Setup tab. You can find the source setup in the Inspector tab by clicking "Manage Sources" or navigate to the "Connection Setup" tab directly.

[comment]: # (todo add screenshot with the buttons) 

The Connection Setup tab shows a list of all sources that have already been created and the status of the Inspector setup. Once Inspector has been successfully installed, the set up status will be updated accordingly.
...
[comment]: # (todo update screenshot to new source overview) 

<center>
  <img
    src={require('../../images/workspace/inspector/inspector-source-states.png')}
