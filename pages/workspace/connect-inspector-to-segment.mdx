...
Pick an event. Keep in mind that the reused Segment event will have the same timestamp as it get when it was fired, so we recommend to pick a recent event.
