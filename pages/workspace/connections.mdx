...
platform, programming language and <Link href="/concepts/destinations"><a>Destinations</a></Link> to send to.
...
It's also sometimes possible to use a custom destination for language - analytics pairs that are not supported by Avo yet. Please <Link href="/help/troubleshooting"><a>contact us</a></Link> if you are unsure of what is the best solution for you.

Detailed instructions on how to set up a custom destination can be found in our <Link href="/custom-destinations"><a>custom destination docs</a></Link>.
