...
- <a href="#snowplow">Snowplow Data Structures (beta)</a>
...
The [mParticle integration](https://docs.mparticle.com/guides/data-master/introduction/) enables you to publish your Avo Tracking Plan into mParticle. You can configure the integration to publish whenever a branch is merged or with a push of a button.
...
- **mParticle Client ID**: Your client ID issued by mParticle
- **mParticle Client Secret**: Your client secret issued by mParticle
- **mParticle Workspace ID**: The ID of the workspace containing your data plans
- **mParticle Plan ID**: The ID of the Data Plan to update

### Publishing to mParticle

Avo will override all events in the mParticle Tracking Plan that are being imported from Avo. Any changes made to an event in mParticle Tracking Plan between Avo publish will be overridden on next Avo publish.
...
<img
  src={require('../../../images/publishing_snowplow.svg')}
  alt="Snowplow"
  style={{ margin: '32px 0 -32px 0' }}
/>

## <a name="snowplow"></a> Snowplow Data Master

> Snowplow integration is currently in beta, <Link passHref href="/help/troubleshooting"><a>contact us</a></Link> to get access
...
To enable the direct integration with Snowplow Data Structures, you need to configure it with the following parameters from Snowplow.

- **Snowplow Organization ID**: Your organization ID issued by Snowplow
- **Snowplow Vendor Name**: The vendor name for the data structures published by Avo, usually your domain in revers domain notation
- **Snowplow Access Token**: The API Token provided by Snowplow for authentication

### Publishing to Snowplow

Avo will upload the events to the Development environment of your Snowplow workspace. You'll need to manually promote them to Production in the [Snowplow console](https://docs.snowplowanalytics.com/docs/understanding-tracking-design/managing-data-structures#promote)
