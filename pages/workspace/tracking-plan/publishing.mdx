...
To create a new publishing integration, find the Publish part of the tracking plan in the left side navigation bar. From there you can click the "Add Integration" button to create a new publishing integration.
...
Note that the publishing integrations are independent of the destinations defined in the "Connection Setup" tab. However the publishing integration can use those destinations to filter which events should be included in the publishing integration. See more below in <Link passHref href="/workspace/tracking-plan/publishing#filtering-events-for-publishing"><a>Filtering events for publishing</a></Link>.
