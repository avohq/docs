...
Note that the publishing integrations are independant of the destinations defined in the "Conenction Setup" tab. However the publishing integration can use those destinations to filter which events should be included in the publishing integration. See more below in <Link passHref href="/workspace/tracking-plan/publishing#filtering-events-for-publishing"><a>Filtering events for publishing</a></Link>.
