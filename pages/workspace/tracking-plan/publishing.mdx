...
To see your exact Webhook payload choose `Tracking plan` - `Publishing` in the sidebar, then pick you Webhook in the menu and expand the `Payload preview` section.
...
| developmentPlatform | string        | Name of development platroft platform (e.g. "Web", "Android") |
...
| type      | string | Name of the provider (e.g. "Segment", "Mispanel") where the data is sent to if managed by Avo, "Custom" otherwise. |
...
| integrationId   | string           |    Id of the publish integration (conistently the same for this integration) |
...
| publishMethod   | { "type": type } | Desctiprion of the publish method, type can be "Manual" and "BranchMerge" |
