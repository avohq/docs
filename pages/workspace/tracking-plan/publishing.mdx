...
## <a name="rudderstack"></a> RudderStack

The integration to [RudderStack](https://rudderstack.com/blog/dogfooding-at-rudderstack-tracking-plans-part-1/) allows for your Tracking Plan to be published into RudderStack whenever branches are merged in your workspace or with a push of a button.

### Configure the RudderStack integration

To enable the direct integration with Segment Protocols, you need to configure it with the following parameters from RudderStack.
...
Avo will not override any events that are only defined in RudderStack, or events that have not been added to the RudderStack integration in Avo.
