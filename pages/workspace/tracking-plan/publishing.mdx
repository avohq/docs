...
Some events that are in your Govern Schema are not recognized by the Govern API, for example the merged events.
Events that end up returning 409 error in Amplitude are likely not active. Please double check them and remove them from your Avo tracking plan if they are not active to not get this warnings.
...
We've exceeded the number of Govern requests we can make per hour for these API credentials or there are too many concurrent requests to the Amplitude Govern API with given credentials.
