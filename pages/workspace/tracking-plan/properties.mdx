...
Properties are also known as traits, attributes, metadata, etc. They are information that is attached to events, users or groups that describe the state of the user or the product at the time of the event. Avo allows you to define event, user and system properties - each category described below.
...
Group properties describe the state of a group you defined previously. Read more about Groups <Link passHref href="/data-design/groups"><a>here</a></Link>.
Similar to User properties, this information is usually relevant in a larger scope than for exactly that event.

**Example**

For example, you are a B2B company and your product is used by different companies and each company consists of a number of users.
You can maintain the state of each company with group properties, increase the count of employees when a new users joins particular company for example.

**Functionality**

Group properties are updated with the event in the LINK Update Group action.
