...
### <a name="property-presence-per-event"></a> Marking properties optional per event

Properties can be marked as optional. That means that Avo will not give you an error if that property is missing from an event that the property is attached to, in other words it will sometimes be sent.
...
### <a name="property-presence-per-source-and-event"></a> Marking properties optional depending on source

Properties can also be marked optional depending on source. As in the scenario above this additionally means Avo will not give you an error if that property is missing from an event which the property is attached to, for a given source.
