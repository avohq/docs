...
Event property groups are a way to group two or more related event properties to quickly and consistently add them to all related events.
...
- object _(<Link passHref href="/help/troubleshooting"><a>reach out to us</a></Link> for access)_
...
To replace a property with another property, you can use the `Replace property with...` (see screenshot) action from the context menu.
This makes sure that the replaced property is also replaced in the events where it is used.

<center>
  <img
    src={require('../../../images/replace-property-with-feature.png')}
    width="100%"
    alt="Image shows the option to replace property with another property from a propertie's context menu"
  />
</center>
