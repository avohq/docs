...
> Inspector monitors events from all branches, so it may report 'Sometimes sent correctly' for the correctly implemented events if old version is still sent from main or other branches.
> In order to make sure the inspector is reporting correctly for your branch, we suggest to decrease the time window to have it catch only a time where you are triggering the events changed.
