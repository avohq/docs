...
The timestamp(Unix timestamp) is prefixed by `ts=` and the signature is prefixed by `v1`.

```json
Avo-Signature:ts=1651677541,v1=30703093db3c532c6f0ab1f1f9eb80b134f5bf6e2ff7064c403aab7582df0667
```
...
Split the header on comma (`,`) as the separator to get a list of elements. Then split each element using the `=` sign to get a key and value dict. 
...
The `signed_payload` string is created by concatenating
 ...
 ```pseudocode
  var signed_payload = 
  requestTimestamp // Timestamp from Avo-Signature header
  ++ "." 
...
Compute an HMAC using the SHA-256 hash function. Use the webhooks endpoint signing secret from your Webhook publishing page in Avo as the key, and use the `signed_payload` string as the message.


```pseudocode
var signature = hmac_sha256(~key=<Secret-from-avo-dashboard>, ~message=signed_payload);
```
...
```pseudocode
if (signature === requestSignature) {   // requestSignature is the v1 signature from the header.
  ...// handle request
}
else {
